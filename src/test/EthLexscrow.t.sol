...
contract EthLexscrowTest is Test {
    ...
    function setUp() public {
        EthLexscrow.Amounts memory _amounts = EthLexscrow.Amounts(deposit, totalAmount, fee, receiver);
    ...
    function testReceive(uint256 _amount) public payable {
        ...
        if (
            _amount > totalAmountWithFee || escrowTest.expirationTime() <= block.timestamp || escrowTest.rejected(buyer)
        ) vm.expectRevert();
        (_success, ) = escrowTestAddr.call{value: _amount}("");
        if (_amount > escrowTest.deposit() && _amount <= escrowTest.totalWithFee() && _success)
            assertTrue(escrowTest.deposited(), "deposited variable did not update");
        if (escrowTest.openOffer()) {
            assertTrue(escrowTest.buyer() == payable(msg.sender), "buyer variable did not update");
        }
    ...
    function testCheckIfExpired(uint256 timestamp) external {
        ...
        if (escrowTest.expirationTime() <= timestamp) {
            ...
            if (escrowTest.refundable())
                ...
                uint256 _remainder = escrowTestAddr.balance - escrowTest.deposit();
                assertEq(
    ...
    function testRejectDepositor(uint256 _deposit) external {
        ...
        EthLexscrow.Amounts memory _amounts = EthLexscrow.Amounts(deposit, totalAmount, fee, receiver);
        openEscrowTest = new EthLexscrow(true, true, expirationTime, seller, buyer, address(0), _amounts);
    ...
    function testWithdraw(address _caller) external {
        ...
        if (!_reverted) {
            assertGt(_preBalance, escrowTestAddr.balance, "balance of escrowTest not affected");
    ...
    function testExecute(uint256 _timestamp, uint256 _deposit, uint256 _totalAmount, uint256 _fee) external {
        ...
        fuzzEscrowTest = new EthLexscrow(true, true, _timestamp, seller, buyer, address(0), _amounts);
    ...
    function testConditionedExecute(bool[] calldata _op) external {
        ...
        conditionEscrowTest = new EthLexscrow(true, true, expirationTime, seller, buyer, address(_manager), _amounts);
        ...
        if (_approved && !conditionEscrowTest.isExpired()) {
            ...
            assertEq(address(conditionEscrowTest).balance, 0, "escrow balance should be zero");
