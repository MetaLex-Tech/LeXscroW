...
contract TokenLexscrowTest is Test {
    ...
    function testConstructorFuzz(
        ...
        TokenLexscrow.Amounts memory _amounts = TokenLexscrow.Amounts(_deposit, _totalAmount, 0, receiver);
        bool _reverted;
        if (
        ...
        TokenLexscrow fuzzDeploy = new TokenLexscrow(
            _refundable,
            _openOffer,
            _expirationTime,
            _seller,
        ...
        if (!_reverted) {
            ...
            assertEq(fuzzDeploy.tokenContract(), testTokenAddr, "token contract mismatch");
        }
    }
